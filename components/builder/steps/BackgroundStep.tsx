...
// Minimal shape needed for the builder display — compatible with both SRD and homebrew.
...
export function BackgroundStep() {
  ...
  const srdBgs: DisplayBackground[] = SRD_BACKGROUNDS.map((bg) => ({
    ...bg,
    isHomebrew: false,
  }));

  const hbBgs: DisplayBackground[] = homebrewBackgrounds.map((bg) => ({
    slug: bg.slug,
    name: bg.name,
    // Use the feature description as the background overview text.
    desc: bg.feature.desc,
  ...
  const allBackgrounds: DisplayBackground[] = [...srdBgs, ...hbBgs];
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Your background defines your character&apos;s history and provides skill proficiencies
              ...
              >
                <div>
                  ...
                  <span className="text-xs text-muted-foreground">
                    ...
                    {bg.languages > 0 &&
                      ` · ${bg.languages} language${bg.languages > 1 ? "s" : ""}`}
